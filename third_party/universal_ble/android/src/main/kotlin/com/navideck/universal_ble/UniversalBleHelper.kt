...
object UniversalBleShutdown {
    fun disconnectAll(reason: String = "shutdown") {
        val gatts = synchronized(knownGatts) {
            knownGatts.values.toList().also {
                knownGatts.clear()
            }
        }
        if (gatts.isEmpty()) {
            Log.i(TAG, "No known BLE GATT connections to close: $reason")
            return
        }

        Log.i(TAG, "Closing ${gatts.size} BLE GATT connection(s): $reason")
        gatts.forEach { gatt ->
            gatt.disconnectAndClose(reason)
        }
    }
}
...
fun String.findGatt(): BluetoothGatt? {
    return synchronized(knownGatts) {
        knownGatts[this]
    }
}

fun BluetoothGatt.saveCacheIfNeeded() {
    synchronized(knownGatts) {
...
fun BluetoothGatt.disconnectAndClose(reason: String = "disconnect") {
    val address = try {
        device.address
    } catch (_: Throwable) {
        "unknown"
    ...
    try {
        ...
        Log.w(TAG, "Failed to disconnect BLE GATT $address", error)
    }
    try {
        close()
        Log.i(TAG, "Closed BLE GATT $address: $reason")
    } catch (error: Throwable) {
        Log.w(TAG, "Failed to close BLE GATT $address", error)
    }
}
